...
tuna_catch_condensed <- data.frame(tuna_catch_top8) %>% 
  ...
  mutate(stock_name = case_when(stockid == "YFINATL" ~ "Yellowfin N. Atl.",
                                stockid == "BIGEYECWPAC" ~ "Bigeye W. Pac.",
                                stockid == "BIGEYEIO" ~ "Bigeye Ind.",
                                stockid == "SKJCIO" ~ "Skipjack Ind.",
                                stockid == "SKJCWPAC" ~ "Skipjack W. Pac.",
...
```{r}
tuna_catch_plot <- ggplot(tuna_catch_condensed, aes(x = year, y = catch, fill = stock_name)) +
  geom_area() +
